...
const cupVersion=(await(await read()).json()).records.find(r=>r.productId==='cup').version;
const incomingCup={id:replayId,productId:'cup',action:'incoming',version:cupVersion,quantity:12};
assert.equal((await send(inv,incomingCup)).status,200);
assert.equal((await(await send(inv,incomingCup)).json()).replayed,true,'Identical request may replay safely.');
assert.equal((await send(inv,{...incomingCup,quantity:24})).status,409,'An ID must not accept a different quantity.');
assert.equal((await send(inv,{...incomingCup,action:'count'})).status,409,'An ID must not accept a different action.');
assert.equal((await send(inv,{...incomingCup,productId:'milk'})).status,409,'An ID must not accept a different product.');
assert.equal((await(await read()).json()).records.find(r=>r.productId==='cup').incoming,12,'Conflicting retry must leave stock unchanged.');
const oldEvent=sql.prepare('SELECT data FROM inventory_events WHERE company_id=? AND id=?').get('company-a',replayId);
const historicalEvent=JSON.parse(oldEvent.data);
delete historicalEvent.request;
sql.prepare('UPDATE inventory_events SET data=? WHERE company_id=? AND id=?').run(JSON.stringify(historicalEvent),'company-a',replayId);
assert.equal((await send(inv,incomingCup)).status,409,'Events saved before request details existed cannot be verified as identical.');
...
sql.prepare('INSERT INTO memberships VALUES (?,?,?)').run('employee-a','company-a','employee');
globalThis.testUser={userId:'employee-a',email:'employee@example.test'};assert.equal((await send(inv,{...incomingCup,id:crypto.randomUUID()})).status,403,'Employee cannot mutate inventory.');
globalThis.testUser=null;assert.equal((await read()).status,401);assert.equal((await send(sales,sale)).status,401);assert.equal((await send(inv,{...incomingCup,id:crypto.randomUUID()})).status,401);
