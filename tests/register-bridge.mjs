...
assert.equal((await(await regGet()).json()).settings.provider,'square');
assert.equal((await send(register,{action:'save',settings:{provider:'other',customName:'',location:''}})).status,400);
const tokenData=await(await send(register,{action:'token'})).json();assert.ok(tokenData.token);
...
globalThis.testUser={userId:'outsider'};assert.equal((await send(register,{action:'token'})).status,403);
assert.deepEqual(parseRegisterCsv('provider,location,item_id,quantity\nTest POS,store-1,latte-small,2',new Set([key])),{[key]:2});
