...
const input={companyId:'a',productId:'milk',actor,sales,supplier,priceEstimate};
...
assert.equal(snapshot.supplier.mappingId,'fake-mapping');
assert.equal(snapshot.supplier.mappingVersion,1);
assert.deepEqual(snapshot.priceEstimate,{source:'fictional_fixture',currency:'USD',perPackMinor:'725'});
assert.deepEqual(snapshot.estimatedLineTotal,{currency:'USD',minor:'1450'});
...
await assert.rejects(review.build({...input,priceEstimate:{...priceEstimate,companyId:'b'}}),/price fixture must belong/);
...
await assert.rejects(review.build({...input,supplier:{...supplier,mappingVersion:0}}),/mapping version is invalid/);
await assert.rejects(review.build({...input,priceEstimate:{...priceEstimate,perPackMinor:'1.25'}}),/price fixture is invalid/);
assert.equal((await review.build({...input,actor:{...actor,role:'employee'}})).kind,'snapshot');
assert.deepEqual(await review.build({...input,companyId:'b',actor:{companyId:'b',userId:'fictional-b',role:'manager'},sales:{...sales,companyId:'b'},supplier:{...supplier,companyId:'b'},priceEstimate:{...priceEstimate,companyId:'b'}}),{kind:'unavailable',reason:'settings_missing'});
const unpriced=await review.build({...input,priceEstimate:null});
assert.equal(unpriced.snapshot.estimatedLineTotal,null);
assert.ok(unpriced.snapshot.explanation.reviewReasons.includes('price_not_checked'));
