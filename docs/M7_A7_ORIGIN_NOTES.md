...
Migration `0015` adds only the empty `replenishment_proposal_origins` table,
...
Local verification on the A7 branch passed TypeScript, 26 test suites, the
16-migration schema check, the application build, Wrangler's local D1 apply of
`0015`, and the local HTTP smoke test. The focused origin test also covers a
