...
- PENDING: Ubuntu and Windows repository checks on the B5 branch, followed by main-branch checks after merge.

## Acceptance and handoff

B5/M4 remains pending until repository CI passes. The B6 handoff is the accepted `pantrack.sales.v1` service and `pantrack.inventory-consumption.v1` port; Clover adapter authentication, merchant/location binding, cursor reconciliation, token lifecycle, and sandbox cases belong to B6/B7. Hosted development D1 migration `0012` and Worker smoke remain separate gates before remotely enabling the exact preview. Keep supplier submission and automatic purchasing disabled.
