...
Migration `0016` adds company-scoped proposal state and append-only audit events.
It backfills one state and creation event for every `0015` origin without changing
...
Local checks on the A7 lifecycle branch passed: TypeScript `--noEmit`, all 28
test suites, the 17-migration schema/journal/snapshot check, the application
build, Wrangler's `DB --local` apply of `0016`, and the served HTTP smoke test.
...
For local recovery before any post-`0016` origin or state change, restore a
pre-`0016` local database snapshot. After such a change, retain history
