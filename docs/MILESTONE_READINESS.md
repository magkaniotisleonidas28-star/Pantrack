...
| M7: Proposals | A6 and A7 review-only lifecycle/handoff complete locally; milestone still open | M5 reliable inputs; A8 integration, UI and end-to-end acceptance. Development D1 still needs the A7 migrations before hosted use. | Target math and warnings visible in UI; concurrent proposal generation/approval and inventory-change tests; stale inputs require review; unknown submissions block duplicates; every edit has actor and reason. Keep implementation in review mode. |
