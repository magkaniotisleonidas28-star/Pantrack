...
| M2: Authentication and permissions | Implemented on `main`; acceptance pending | Supabase development setup, real email/login/recovery verification, failed Cloudflare build diagnosis, and migration/authentication review. See [M2 evidence](M2_LOCAL_EVIDENCE.md) and [M2 setup](M2_SETUP.md). | Anonymous, cross-company, every-role/API-family, CSRF, session-expiration, invitation replay/expiration, logout, and last-owner tests; approved authentication decision record. |
...
| M7: Proposals | A6 review-only core complete locally; milestone still open | M5 reliable inputs; A7 approval/edit audit, lifecycle states, invalidation, and atomic unresolved-quantity reservations; A8 integration and acceptance. | Target math and warnings visible in UI; concurrent proposal generation/approval and inventory-change tests; stale inputs require review; unknown submissions block duplicates; every edit has actor and reason. Keep implementation in review mode. |
