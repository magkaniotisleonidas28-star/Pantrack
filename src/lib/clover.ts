...
// Workers rejects redirect:'error'; manual prevents token-bearing requests from following redirects.
export async function cloverJson(url:string,init:RequestInit={}){const r=await fetch(url,{...init,redirect:'manual',headers:{'User-Agent':'Pantrack/1.0',...init.headers},signal:AbortSignal.timeout(15000)});if(!r.ok)throw new CloverHttpError(r.status);const text=await r.text();if(text.length>2000000)throw new Error('Clover response is too large.');return JSON.parse(text) as unknown;}
