...
export async function GET(){
 ...
 try{
 const db=database();
 // Preserve the prior user-owned workspace without copying or reassigning anyone else's data.
 const legacy=await db.prepare('SELECT id FROM products WHERE owner=? LIMIT 1').bind(user.userId).first();
 if(legacy)await db.batch([
 db.prepare('INSERT OR IGNORE INTO companies(id,name,created) VALUES (?,?,?)').bind(user.userId,'My company',new Date().toISOString()),
 db.prepare('INSERT OR IGNORE INTO memberships(user_id,company_id,role) VALUES (?,?,?)').bind(user.userId,user.userId,'owner')]);
